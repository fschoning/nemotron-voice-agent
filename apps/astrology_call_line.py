...
INTERNALWS_BASE_URL = os.getenv("INTERNALWS_BASE_URL", "http://192.168.1.121:17080/internalws")
...
async def run_bot(transport: DailyTransport, runner_args: RunnerArguments, session_data: dict = None, tenant: str = None, appt_uid: str = None):
    ...
    if not guardrail_prompt_text or not guardrail_prompt_text.strip():
        logger.warning("⚠️ 'guardrailPrompt' is missing or empty. Bypassing guardrail prompt validation.")
